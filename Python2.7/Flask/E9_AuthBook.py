...
4.蓝图：拆分模块;函数钩子：交错导入
    ...
    from _009auth_book import Author, Book, db, redirect, url_for
    2.使用蓝图对象
    @api.route()
    3.绑定蓝图到应用实例上
...
# 配置数据库连接
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://guest:guest@localhost/004test'
...
def get_auth_book():
    ...
    if form.validate_on_submit():
        ...
        auth = Author.query.all()
        book = Book.query.all()
    return render_template('003index.html', auth=auth, book=book, form=form)
