...
def index(filename, url):
    """股票主页"""
    try:
        fd = open(g_templates_root + filename, encoding="utf-8")
        ...
        db = connect(host='localhost', port=3306, database='miniweb', user='guest', password='guest', charset='utf8')
        cs = db.cursor()
...
def update(filename, url):
    """个人信息中心"""
    try:
        fd = open(g_templates_root + '/update.html', encoding="utf-8")
    except Exception as e:
        return '%s' % e
    else:
        content = fd.read()
