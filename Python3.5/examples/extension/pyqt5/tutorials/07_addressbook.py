import sys
from PyQt5.QtWidgets import QApplication, QWidget, QLabel, QLineEdit, QTextEdit, QGridLayout, QPushButton, QVBoxLayout, QMessageBox, QHBoxLayout, QDialog, QFileDialog
...
class AddressBook(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.findbutton = QPushButton('&Find')  # 设置查找按钮对象
        ...
        self.loadbutton = QPushButton("&Load...")  # 设置导入文件
        self.loadbutton.setToolTip("Load contacts from a file")
        self.savebutton = QPushButton("Sa&ve...")  # 设置存储文件
        self.savebutton.setToolTip("Save contacts to a file")
        self.savebutton.setEnabled(False)  # 设置默认不可以存储

        self.exportbutton = QPushButton("Ex&port")  # 设置导出文件
        ...
        self.findbutton.clicked.connect(self.findcontact)  # 设置查找事件
        ...
        self.loadbutton.clicked.connect(self.loadfromfile)  # 设置加载文件事件
        self.savebutton.clicked.connect(self.savetofile)  # 设置保存文件事件
        self.exportbutton.clicked.connect(self.export_as_vcard)  # 设置导出文件事件
        ...
        mainlayout.addLayout(buttonlayout2, 2, 1)  # 增加HB布局按钮
    ...
    def findcontact(self):
        ...
        if self.dialog.exec_() == QDialog.Accepted:
            ...
            if contactname in self.contacts:  # 找到数据则在编辑框内显示
                self.nameline.setText(contactname)
                self.addresstext.setText(self.contacts[contactname])
            else:
                QMessageBox.information(self, "Contact Not Found", "Sorry, \"%s\" is not in your address book." % contactname)
                return
    ...
    def updateinterface(self, mode):
        ...
        if self.currentmode in (self.addingmode, self.editingmode):
            ...
            self.loadbutton.setEnabled(False)  # 设置加载按钮不可用
            self.savebutton.setEnabled(False)  # 设置保存按钮不可用
            self.exportbutton.setEnabled(False)  # 设置导出按钮不可用
            ...
            self.loadbutton.setEnabled(True)  # 设置加载按钮可用
            self.savebutton.setEnabled(number >= 1)  # 设置保存按钮可用
            self.exportbutton.setEnabled(number >= 1)  # 设置导出按钮可用
    ...
    def loadfromfile(self):
        filename, _ = QFileDialog.getOpenFileName(self, "Open Address Book", "", "Address Book (*.abk)l;;All Files (*)")
        ...
        self.contacts = pickle.load(in_file)  # 读取文件内容加入通讯录
        in_file.close()

        if len(self.contacts) == 0:
            QMessageBox.information(self, "No contacts in file", "The file you are attempting to open contains no contacts.")
        else:
            for name, address in self.contacts:  # 行编辑框和文本编辑框显示
    ...
    def export_as_vcard(self):
        ...
        filename, _ = QFileDialog.getSaveFileName(self, "Export Contact", "", "vCard Files (*.vcf);;All Files (*)")
