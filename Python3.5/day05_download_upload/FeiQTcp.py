...
def get_file_info(file_info):
    ...
    file_id = file_list[1]
    return int(packet_id, 16), file_id
...
def send_file(client_socket):
    ...
    packet_id, file_id = get_file_info(feiq_dict)
    ...
    for i, file_info in enumerate(g_file_infos):
        if file_info['packet_id'] == packet_id and file_info['file_id'] == file_id:
            ...
            try:
                ...
                print('文件发送成功...')
                udp_FeiQCoreData.file_info_queue.remove(file_info)
                print('列表删除成功...')
            break
    else:
        print('no file found!')
...
def get_file_info_from_queue(file_info_queue):
    while not file_info_queue.empty():
