...
def error500(request):
    # 产生500错误
    1 + 's'
    return HttpResponse("view应用产生500错误!")


# 自定义404错误处理
def handle404(request):
    return HttpResponse("view应用处理404错误!")


# 自定义500错误处理
def handle500(request):
    return HttpResponse("view应用处理500错误!")


# 自定义403错误处理
def handle403(request):
    return HttpResponse("view应用处理403错误!")
...
def urlreverse(request, param):
    ...
    return HttpResponse("这是url不定长参数反向解析: " + str(redirect_url))


# url关键字参数反向解析
def urlreversekey(request, num):
    print(num)
    redirect_url = reverse('view:urlreversekey', kwargs={'num': 2})
    print("重定向地址: ", redirect_url)
    return HttpResponse("这是url关键字参数反向解析: " + str(redirect_url))
...
def urlgetsolve(request):
    urlgets = [
        ("请求路径: ", request.path),
        ("请求方法: ", request.method),
        ("请求get所有参数: ", request.GET),
        ("请求post所有参数: ", request.POST),
        ("请求files参数: ", request.FILES),
        ("请求cookies参数: ", request.COOKIES),
        ("请求session参数: ", request.session),
    ]
    html = '<h1>处理get请求</h1><hr>'
    for items in urlgets:
        print(items[0], items[1])
        html += '<h3>' + str(items[0]) + str(items[1]) + '</h3>'
    html += '<hr>'
    return HttpResponse(html)
...
# 产生post请求
def urlpost(request):
    return render(request, 'view/post.html', locals())
...
def urlpostsolve(request):
    ...
    html = '<h1>处理post请求</h1><hr>'
    for items in urlposts:
        print(items[0], items[1])
        html += '<h3>' + str(items[0]) + str(items[1]) + '</h3>'
    html += '<hr>'
    return HttpResponse(html)
...
def urlresponse(request):
    ...
    print("HttpResponse响应的内容: ", response.getvalue().decode('utf-8'))
    return response
...
def ajax(request):
    ...
    if a and b:
        ...
        return render(request, 'view/ajax.html', locals())
...
def cookieget(request):
    ...
    return HttpResponse('获取cookie的name值: ' + str(cookie))


# 删除cookie
def cookiedel(request):
    response = HttpResponse("删除cookie结束!")
...
def sessionget(request):
    ...
    return HttpResponse('获取session的name值: ' + str(session))
