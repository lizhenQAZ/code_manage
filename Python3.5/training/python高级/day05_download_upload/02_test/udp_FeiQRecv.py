    ...
    while True:
        ...
        if recv_data and dest_addr:
            ...
            if udp_FeiQCoreData.IPMSG_BR_ENTRY == command:
                ...
                if command_option & 0x00f00000 == udp_FeiQCoreData.IPMSG_FILEATTACHOPT:
                    # 飞鸽传书中的选项为  \0 文件序号:文件名:文件大小:文件修改时间:文件类型:
                    file_info_msg = dict()
                    find_zero_position = data_dict['option'].find('\0')
                    # 跳过\0, 当然了如果对方发送过来的还包括普通的消息，那么就需要find查找
                    file_info_list = data_dict['option'][find_zero_position+1:].split(":", 5)
                    file_info_msg['packet_id'] = int(data_dict['packet_id'])
                    ...
                    udp_FeiQCoreData.download_file_list.append(file_info_msg)

                else:
                    print("%s(%s)>>>%s" % (data_dict['username'], str(dest_addr), data_dict['option']))
